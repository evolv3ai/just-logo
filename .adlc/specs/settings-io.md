...
- `src/lib/settings-io.ts` is a pure module (no React, no DOM) with the whole editor↔spec mapping:
  ...
  - `importSettingsFile(file, iconList)` holds every decision the import button makes, so the component only shows the message or applies the result. `file` is the `{ size, text() }` part of a browser `File`; `iconList` is `pending`, `error` or `ready` with the icons. It refuses, without reading the file, while the icon list is pending (try again), when it failed to load (reload the page; waiting will not help because `useIcons` does not retry), and when `size` is over `MAX_SETTINGS_BYTES` (65536). A read error is a refusal too. Otherwise it returns `parseSettings` of the text.
  - `editorToSpec` also refuses a stored state that lacks a field (`<key>: missing`): `validateSpec` reads `undefined` as absent and `JSON.stringify` would drop the key, so the CLI would fill it from its own defaults and draw something other than the preview.
- `src/components/editor/editor-header/settings-menu.tsx` adds two outline icon buttons with tooltips, in their own separator group after Randomize and Clear in the header, matching the existing buttons: **Export settings** (hotkey `J`) downloads `logo.json`; **Import settings** (hotkey `I`) opens a hidden `<input type="file" accept="application/json,.json">`. The change handler passes the picked file and the state of `useIcons` (`data`, `isError`) to `importSettingsFile` and makes no decision of its own. Success and every refusal are shown with the existing `sonner` toast. A successful import calls `updateIconSettings` and `updateBackgroundSettings` with the full mapped state, so autosave and the undo history pick it up the same way they pick up Randomize. A refused import calls neither.
...
3. **AC3, export refusals.** `editorToSpec` with `icon: null` returns `ok: false` with a message telling the user to pick an icon; with an out-of-range value (`size: 9999`) it returns `ok: false` carrying the validator's message; with a state that lacks fields it returns `ok: false` naming each as `<key>: missing`. Verified by unit tests.
