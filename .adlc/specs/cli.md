...
- Rendering is pure string composition. The output SVG mirrors what the editor's export zone contains: a 512x512 canvas, a background box with the editor's background, margin, corner radius and border, and the chosen icon centred with the editor's size, rotation, stroke and fill settings. The border follows the CSS box model: it sits inside the box, keeps the outer radius, and the background fills the area inside it. `linear-gradient(...)` (any CSS angle unit or side keyword) is converted to an SVG `<linearGradient>` with CSS corner-to-corner geometry and CSS stop-position rules; `radial-gradient(...)` becomes a centred `<radialGradient>` with the default radius, and if a shape, size or position prefix was dropped the result is flagged `backgroundApproximated: true`. A gradient with a non-colour stop is not converted. Any other background string is written as-is, flagged `backgroundPassthrough: true`, and warned about on stderr.
...
Every command accepts `--json`. With it, stdout is exactly one JSON value and nothing else. Every command rejects unknown flags. Exit codes: 0 success, 1 operational error (config file unreadable or missing, missing icon, raster failure), 2 usage error (unknown flag, bad flag value, malformed or invalid config content). Errors go to stderr as `error: <what>` followed by `help: <a runnable fix>`; user-supplied text in those lines is reduced to one line with control characters removed, so a config value cannot forge a `help:` line. Numeric flags reject empty values; `--limit` must be a whole number of 1 or more. String values in a spec are at most 512 characters.
